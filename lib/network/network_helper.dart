...
import 'package:practice/network/network_typedef.dart';
...
class NetworkHelper {
  ...
  static String concatUrlQP(String url, Map<String, String>?queryParameters){
    ...
    queryParameters.forEach((key, value) {
      ...
      if (value.contains(' ')) throw Exception('Invalid input Exception');      // the main point of the error
  ...
  static R filterResponse<R>({
    ...
    required NetworkFailureCallBackWithMessage onFailureCallBackWithMessage,
    CallBackParameterName parameterName = CallBackParameterName.all 
  }){
    try{
    if (response == null || response.body.isEmpty ){
      return onFailureCallBackWithMessage(NetworkResponseErrorType.responseEmpty,'empty response'); // the other point of the error
    }
